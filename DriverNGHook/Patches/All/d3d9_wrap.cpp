...
WrappedD3DDevice9::WrappedD3DDevice9(IDirect3DDevice9* device, HWND wnd, DriverNG::IDirect3DDelegate* delegate)
...
void WrappedD3DDevice9::CheckForDeath()
{
    ...
    {
        assert(m_SoftRefCounter.GetRefCount() >= m_InternalRefcount);
...
HRESULT __stdcall WrappedD3DDevice9::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
{
    ...
    m_drawDelegate->OnDeviceLost();
    auto result = m_device->Reset(pPresentationParameters);
    m_drawDelegate->OnReset(m_device);
    m_drawDelegate->OnDeviceRestored(m_device);
    ...
    HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
{
    ...
    if (wnd != NULL)
    {
        ...
        if (m_drawDelegate)
        {
            m_drawDelegate->OnBeginScene(m_device);
            m_drawDelegate->OnEndScene(m_device);
        }
